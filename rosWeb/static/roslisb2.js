...
//rescuer marker
var rescuerMarker = new ROS2D.ArrowShape({
  size : 0.2,
  strokeSize : 0.2,
  pulse : false,
...
var distanceTopic = new ROSLIB.Topic({
  ros : ros,
  name : '/distance',
  messageType: '/tf2_msgs/Int32',
  //messageType:'tf/tfMessage'
});

distanceTopic.subscribe(
  function (distance) {
  let x = Math.sin(degree) * distance;
  let y = Math.sqrt(Math.pow(distance) - Math.pow(x));
  
  rescuerMarker.x = posx + x;
  rescuerMarker.y = posy + y;
  rescuerMarker.rotation = 90;
});
