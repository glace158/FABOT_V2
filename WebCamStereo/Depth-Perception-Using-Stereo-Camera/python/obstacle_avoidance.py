...
def obstacle_avoid():
	...
	if np.sum(mask)/255.0 > 0.01*mask.shape[0]*mask.shape[1]:
		...
		if cv2.contourArea(cnts[0]) > 0.01*mask.shape[0]*mask.shape[1]:
			...
			cv2.putText(output_canvas, "Object at", (x+5,y), 1, 2, (0,0,0), 2, 2)
			#cv2.putText(output_canvas, "%.2f cm"%depth_mean, (x+5,y+40), 1, 2, (0,0,0), 2, 2)
			cv2.putText(output_canvas, "%.2f cm"%depth_mean, (100,100),1,3,(0,255,0),2,3)
...
while True:
	...
	if retL and retR:
		...
		output_canvas = cv2.cvtColor(imgL,cv2.COLOR_BGR2GRAY)
